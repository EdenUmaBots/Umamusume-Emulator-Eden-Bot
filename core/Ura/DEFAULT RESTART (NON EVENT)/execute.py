...
def handle_popups(screenshot):
    """
    Detects and dismisses retry, close, and back popups.
    Returns True if a popup was handled.
    """

    POPUPS = [
        ("assets/buttons/close.png",     0.8, 0.3, "close"),
        ("assets/buttons/back_btn.png",  0.8, 0.3, "back"),
        ("assets/buttons/next2_btn.png", 0.8, 0.3, "next2"),
    ...
    for image_path, confidence, delay, label in POPUPS:
        matches = match_template(screenshot, image_path, confidence=confidence)
        ...
        log_info(f"{label.capitalize()} detected — clicking {image_path} at ({cx}, {cy})")
        tap(cx, cy)
        time.sleep(delay)
...
def start_career():
    ...
    try:
        ...
        # Step 2: Tap Next button three times
        ...
        # Step 5: Tap Friend Support Choose
        ...
        # Step 7A: If not enough TP
        restore_button = match_template(take_screenshot(), "assets/buttons/restore_btn.png", confidence=0.8)
        if restore_button:
            x, y, w, h = restore_button[0]
            center = (x + w//2, y + h//2)
            tap(center[0], center[1])
            ...
            time.sleep(1)
            tap(790, 1385) # OK Button
            log_info(f"TP Restored to maximum")
            time.sleep(1)
            tap(535, 1235) # Close Button
            time.sleep(1)
            tap(585, 1600) # Start Career Button
            time.sleep(0.5)
            tap(800, 1800) # Second Career Button
            log_info(f"Career Started through step 10A")
            time.sleep(5)
        else:
            pass
        ...
        if start_career_2_matches:
            ...
            center = (x + w//2, y + h//2)
            tap(center[0], center[1])
            log_info(f"Career Started through step 10B")
            time.sleep(5)
        else:
            pass
        ...
        skip_matches = wait_for_image("assets/buttons/skip_btn.png", timeout=90, confidence=0.7)
        if skip_matches:
...
def career_lobby():
    ...
    while True:
        ...
        career_home = match_template(screenshot, "assets/buttons/Career_Home.png", confidence=0.7)
        if career_home:
            start_career()
            continue
        ...
        # If Connection Error Popup
        log_debug(f"Checking for connection error...")
        connection_error_matches = match_template(screenshot, "assets/buttons/connection_popup.png", confidence=0.6)
        if connection_error_matches:
            log_info("Connection error popup — tapping fixed coordinates to dismiss popup")
            tap(786, 1240)
            time.sleep(2.0)     
            continue
        ...
        if tazuna_hint is None:
            ...
            tap(540, 960)  # Tap center to potentially dismiss any lingering popups and stabilize UI          
            continue
